...
describe('adkernel analytics adapter should', () => {
    ...
    it('send events for each bid request', () => {
        ...
        expect(aka.firstCall.args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_ImpRequests']);
        expect(aka.secondCall.args).to.be.eql(['send', 'event', 'pb_adkernel_30164_container-1_ImpRequests']);
        expect(aka.thirdCall.args).to.be.eql(['send', 'event', 'pb_adkernel_~_~_Requests']);
    });

    it('send events for first bid response', () => {
        adkernelAdapter.track({eventType : CONSTANTS.EVENTS.BID_RESPONSE, args : BID_RESPONSE_1});
        expect(aka.getCall(0).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_BidLoad', 0.3]);
        expect(aka.getCall(1).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_BidLoadBucket0300-0400ms', 0.3]);
        expect(aka.getCall(2).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_BidCpm', 12.4995]);
        expect(aka.getCall(3).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_BidCpmBucket>$8', 12.4995]);
    });

    it('send events for second bid response', () => {
        adkernelAdapter.track({eventType : CONSTANTS.EVENTS.BID_RESPONSE, args : BID_RESPONSE_2});
        expect(aka.getCall(0).args).to.be.eql(['send', 'event', 'pb_adkernel_30164_container-1_BidLoad', 0.277]);
        expect(aka.getCall(1).args).to.be.eql(['send', 'event', 'pb_adkernel_30164_container-1_BidLoadBucket0200-0300ms', 0.277]);
        expect(aka.getCall(2).args).to.be.eql(['send', 'event', 'pb_adkernel_30164_container-1_BidCpm', 0.015]);
        expect(aka.getCall(3).args).to.be.eql(['send', 'event', 'pb_adkernel_30164_container-1_BidCpmBucket$0-0.5', 0.015]);
    });

    it ('send event for timed out adapter', () => {
        adkernelAdapter.track({eventType : CONSTANTS.EVENTS.BID_TIMEOUT, args : ['foobar']});
        expect(aka.getCall(0).args).to.be.eql(['send', 'event', 'pb_foobar_0_~_Timeout']);
    });

    it('send events for bid won', () => {
        adkernelAdapter.track({eventType : CONSTANTS.EVENTS.BID_WON, args : BID_WON});
        expect(aka.getCall(0).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_WonCpm', 95]);
        expect(aka.getCall(1).args).to.be.eql(['send', 'event', 'pb_adkernel_15417_container-1_WonCpmBucket>$8', 95]);
    });
});